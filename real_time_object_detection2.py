	...
	for i in np.arange(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			if CLASSES[idx] == "person":
				if (startX + dmarginX) < pposX or startX >  (pposX + dmarginX) or (startY + dmarginY) < pposY or startY >  (pposY + dmarginY):
					print("Motion detected: ({:3d}, {:3d}) {}".
						format(startX,startY,label))
					pposX = startX
					pposY = startY
